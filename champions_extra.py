...
def lambert_w(x: object) -> npt.NDArray[np.float64]:
    x = np.asarray(x, dtype=np.float64)
    return x * np.maximum(0.0, np.exp(np.clip(x, -50.0, 50.0)))


def kelly_criterion(p: object, b: object) -> npt.NDArray[np.float64]:
...
def rsi_momentum(g: object, l: object) -> npt.NDArray[np.float64]:
    g = np.asarray(g, dtype=np.float64)
    l = np.asarray(l, dtype=np.float64)
    return 100.0 * np.clip(g / _safe_div(g + l), -1e4, 1e4)


def logsumexp2(b: object, a: object) -> npt.NDArray[np.float64]:
    b = np.asarray(b, dtype=np.float64)
...
def smoothstep(x: object) -> npt.NDArray[np.float64]:
    x = np.asarray(x, dtype=np.float64)
